...
gameRoutes.route("/games").get(async (req, res) => {
  try {
    ...
    // Only return the data is it has any records
    if (data.length > 0) {
      res.json(data);
    } else {
...
gameRoutes.route("/games/leaderboard/:id").post(async (req, res) => {
  try {
    ...
    const existingPlayer = await db.collection("scores").findOne({
      ...
      playerName: { $regex: new RegExp(`^${req.body.playerName}$`, "i") }, // this will make the name matching case insensitive, e.g., John will be matched to JOhN
    });
